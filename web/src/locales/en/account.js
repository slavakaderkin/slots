export default {
  tabs: {
    bookings: 'Bookings',
    profiles: 'Professionals',
  },
  cta: {
    header: 'Provide services?',
    description: 'Create a profile with services and slots in 5 minutes and accept clients from Telegram.',
    header_noprofile: 'You haven\'t created a profile',
    description_noprofile: 'You have a subscription but still no profile. Create one in 5 minutes and start accepting clients.',
  },
  empty_bookings: 'We\'ll put your future bookings here',
  empty_profiles: 'Your favorite professionals will be waiting here',
  profile: 'Workspace',
}