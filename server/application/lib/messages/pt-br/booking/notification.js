async ({ booking, timezone, isDayly, accountId }) => {
  ...
  const lines = [
    `${isDayly ? '<b>Lembrete: você tem um agendamento hoje.</b>' : '<b>Lembrete: você tem um agendamento em uma hora.</b>'}\n`,
    `<b>Serviço:</b> ${service.name}`,
    `<b>Horário:</b> <u>${lib.utils.toHumanDate(datetime, timezone, 'pt')}</u>`
  ];
  ...
  if (isOwner) inline_keyboard.push( [{ text: 'Entrar em contato com cliente', url: `tg://user?id=${tg}` }]);
  if (!isOwner && isDayly) inline_keyboard.push([{ text: 'Não poderei comparecer :(', callback_data: `booking|cancel|id=${bookingId}` }])
